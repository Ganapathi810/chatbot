...
const ChatInput: React.FC<ChatInputProps> = ({ onSendMessage, isLoading, hasMessages, isNewChat = false, isCollapsed = false }) => {
  ...
  const inputContainerClass = hasMessages 
    ? `fixed bottom-0 right-0 p-3 sm:p-4 ${
      ...
      } bg-gradient-to-t from-gray-900 via-gray-900/95 to-transparent pt-8 sm:pt-12`
          ...
          <div className="relative group">
            <div className="relative">
              <textarea
                ...
                disabled={isLoading}
                className="w-full px-6 py-4 pr-16 bg-gray-800/50 backdrop-blur-sm rounded-full border border-gray-600/50 hover:border-gray-500/50 focus:border-orange-500/50 focus:shadow-lg focus:shadow-orange-500/20 text-white placeholder-gray-400 resize-none outline-none focus:outline-none focus:bg-gray-800/50 min-h-[60px] max-h-[200px] overflow-y-auto text-base transition-colors duration-300"
                rows={1}
              />
              ...
              <button
                ...
                className="absolute right-3 top-1/2 transform -translate-y-1/2 pb-1 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:from-gray-600 disabled:to-gray-700 text-white rounded-full transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-orange-500/25 disabled:hover:scale-100 disabled:hover:shadow-none flex items-center justify-center w-12 h-12"
              >
